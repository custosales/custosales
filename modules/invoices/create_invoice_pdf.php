...
$customerData[] = $Rowc['companyAddress'];
...
// global $LANG['date'];
 global $orderDate;
	...
	$this -> Cell('100',20,'',0,0,'',false);
 	...
 	$this->Image('../../images/logo1.png',130,10,'',15);	 
	$this -> Cell('100',20,'',0,0,'',false);
...
function TermsText() { 
	...
	$this->SetFont('Arial','',12);
	$this -> MultiCell('180',6,utf8_decode($terms));
}
...
function FooterText () {
   ...
   $this -> Cell('100',6,$LANG['internet'].': '.$_SESSION['companyInternet'],0,1,'',false);
	...
	$this -> MultiCell('180','5',utf8_decode('Norsk Verdivurdering Ltd er underlagt taushetsplikt vedrørende kundeopplysninger. Norsk Verdivurderings medarbeidere er alle underlagt samme taushetsplikt. Taushetsplikten omfatter all informasjon overlevert av kunden i tillegg til resultater og/eller vurderinger hvor kundens egne data inngår som beregningsgrunnlag. Taushetsplikten omfatter alle data uansett hvorvidt disse er gitt muntlig, elektronisk eller skriftlig'));
